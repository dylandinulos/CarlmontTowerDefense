...
public class GameScreen extends JPanel implements MouseListener
{
    ...
    public void paintComponent(Graphics g)
    {
        ...
        g.drawImage(schoolImage, 14 * TILE_SIZE, 2 * TILE_SIZE + SHOP_HEIGHT, 4 * TILE_SIZE, 3 * TILE_SIZE, null);

        g.drawImage(fieldImage, 1 * TILE_SIZE, 15 * TILE_SIZE + SHOP_HEIGHT, 7 * TILE_SIZE, 4 * TILE_SIZE, null);
        ...
        for(Enemy enemy : enemies)
        {
            ...
            g.fillRect((int)enemy.x - 8, (int)enemy.y - 16, 16, 4);

            g.setColor(Color.GREEN);

            int healthWidth =(int)((enemy.health / (double)enemy.maxHealth) * 16);

            g.fillRect((int)enemy.x - 8, (int)enemy.y - 16, healthWidth, 4);
        }
        ...
        if(selectedTower != null)
        {
            ...
            g.drawOval(selectedTower.getX() + 12 - radius, selectedTower.getY() + 12 - radius, radius * 2, radius * 2);
        }
        ...
        if(placingTower)
        {
            ...
            if(mouse != null)
            {
                ...
                Tower ghost = new Tower(selectedTowerType, mouse.x - 12, mouse.y - 12);
    ...
    public void mousePressed(MouseEvent e)
    {
        ...
        for(Tower tower : towers)
        {
            if(mouseX >= tower.getX() && mouseX <= tower.getX() + 24 && mouseY >= tower.getY() && mouseY <= tower.getY() + 24)
            {
                selectedTower = tower;
                return;
            }
        ...
        if(!isOnPath(mouseX, mouseY) && !towerExists(row, col) && money >= cost)
        {
            towers.add(new Tower(selectedTowerType, col * TILE_SIZE + 4, row * TILE_SIZE + SHOP_HEIGHT + 4));
            money -= cost;
        }
    }
